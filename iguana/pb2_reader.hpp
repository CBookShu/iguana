...
IGUANA_INLINE void from_pb2_impl(T& val, std::string_view& pb_str,
  ...
  if constexpr (ylt_refletable_v<T>) {
    size_t pos;
    uint32_t size = static_cast<uint32_t>(detail::decode_varint(pb_str, pos));
    pb_str = pb_str.substr(pos);
    if (pb_str.size() < size)
      IGUANA_UNLIKELY {
    ...
    while (!pb_str.empty()) {
      ...
      uint32_t key = static_cast<uint32_t>(detail::decode_varint(pb_str, pos));
    ...
    size_t size = static_cast<size_t>(detail::decode_varint(pb_str, pos));
...
IGUANA_INLINE void from_pb2(T& t, std::string_view pb_str) {
  ...
  while (true) {
    ...
    if (it != map.end()) {
      ...
      if (!pb_str.empty())
        IGUANA_LIKELY {
          key = static_cast<uint32_t>(detail::decode_varint(pb_str, pos));
      ...
      if constexpr (detail::is_has_unknown_fields<T>) {
          ...
          case WireType::LengthDelimeted: {
              auto sz = static_cast<size_t>(detail::decode_varint(pb_str, pos));
          ...
          if (!pb_str.empty()) {
              IGUANA_LIKELY{
                  key = static_cast<decltype(key)>(detail::decode_varint(pb_str, pos));
                  wire_type = static_cast<WireType>(key & 0b0111);
                  field_number = key >> 3;
              }
          }
